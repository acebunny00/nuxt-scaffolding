...
<template>
	<USelectMenu v-model="labels" by="id" name="labels" :options="options" option-attribute="name" multiple searchable creatable>
		<template #label>
		...
		<template #option-create="{ option }">
			...
			<span class="flex-shrink-0 w-2 h-2 mt-px rounded-full -mx-1" :style="{ background: `#${generateColorFromString(option.name)}` }" />
